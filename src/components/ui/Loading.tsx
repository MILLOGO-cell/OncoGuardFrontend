...
export default function Loading({ variant = "spinner", fullscreen = false, overlay = false, className }: Props) {
  const content =
    variant === "spinner" ? (
      <Spinner />
    ...
    return (
      <div
        className={clsx(
          ...
          overlay && "backdrop-blur-sm bg-background/40",
          className
        )}
      >
        <div className={clsx(!fullscreen && "max-w-5xl w-full p-6")}>{content}</div>
      </div>
    );
  }
